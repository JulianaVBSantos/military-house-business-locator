...
{ // funcao interna para copiar os dados e evitar crash
    if (origem != NULL)
    {
...
void carregarLojas(const char *arquivo)
{
    ...
    for (int i = 0; i < tamanhoArray && i < MAX_LOJAS; i++)
    {
        ...
        cJSON *lat = cJSON_GetObjectItem(lojaJSON, "lat");
        cJSON *lng = cJSON_GetObjectItem(lojaJSON, "lng");
        ...
        if (lat != NULL)
        {
            ...
            {
                lojas[i].lat = lat->valuedouble;
            }
            else if (cJSON_IsString(lat))
            {
                lojas[i].lat = atof(lat->valuestring);
            }
            else
            {
                lojas[i].lat = -9999;
        ...
        if (lng != NULL)
        {
            ...
            {
                lojas[i].lng = lng->valuedouble;
            }
            else if (cJSON_IsString(lng))
            {
                lojas[i].lng = atof(lng->valuestring);
            }
            else
            {
                lojas[i].lng = -9999;
...
void salvarLojas(const char *arquivo)
{
    ...
    for (int i = 0; i < totalLojas; i++)
    {
        ...
        cJSON_AddNumberToObject(lojaJSON, "lat", lojas[i].lat);
        cJSON_AddNumberToObject(lojaJSON, "lng", lojas[i].lng);
